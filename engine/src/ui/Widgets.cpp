...
void Label::render(const glm::vec2& screenSize) {
    ...
    if (!m_text.empty()) {
        ...
        f32 textWidth = std::min(size.x * 0.8f, static_cast<f32>(m_text.length()) * 6.0f);
        Renderer2D::drawQuad(
            glm::vec3(bounds.x + size.x * 0.1f + textWidth * 0.5f, pos.y, 0.02f),
            glm::vec2(textWidth, size.y * 0.6f),
            m_style.textColor
        );
    }
...
bool Button::onMouseUp(const glm::vec2& mousePos, const glm::vec2& screenSize) {
    ...
    bool result = Widget::onMouseUp(mousePos, screenSize);
...
void Button::render(const glm::vec2& screenSize) {
    ...
    // Text representation
    if (!m_text.empty()) {
        f32 textWidth = std::min(size.x * 0.7f, static_cast<f32>(m_text.length()) * 6.0f);
        Renderer2D::drawQuad(
            glm::vec3(pos.x, pos.y, 0.02f),
            glm::vec2(textWidth, size.y * 0.4f),
            m_style.textColor
        );
    }
...
void ProgressBar::render(const glm::vec2& screenSize) {
    ...
    // Fill bar
    if (m_progress > 0.0f) {
        f32 fillWidth = (size.x - m_style.borderWidth * 2.0f) * m_progress;
        f32 fillHeight = size.y - m_style.borderWidth * 2.0f;
        glm::vec3 fillPos(
            bounds.x + m_style.borderWidth + fillWidth * 0.5f,
            pos.y,
            0.01f
        );
    ...
    if (m_style.borderWidth > 0.0f) {
        // Top
        Renderer2D::drawQuad(
            glm::vec3(pos.x, bounds.w - m_style.borderWidth * 0.5f, 0.02f),
            glm::vec2(size.x, m_style.borderWidth),
            m_style.borderColor
        );
        // Bottom
        Renderer2D::drawQuad(
            glm::vec3(pos.x, bounds.y + m_style.borderWidth * 0.5f, 0.02f),
            glm::vec2(size.x, m_style.borderWidth),
            m_style.borderColor
        );
        // Left
        Renderer2D::drawQuad(
            glm::vec3(bounds.x + m_style.borderWidth * 0.5f, pos.y, 0.02f),
            glm::vec2(m_style.borderWidth, size.y),
            m_style.borderColor
        );
        // Right
        Renderer2D::drawQuad(
            glm::vec3(bounds.z - m_style.borderWidth * 0.5f, pos.y, 0.02f),
            glm::vec2(m_style.borderWidth, size.y),
            m_style.borderColor
        );
    }
