...
Result<void, String> Texture2D::loadFromFile(const std::filesystem::path& path) {
    ...
    if (!data) {
        return unexpected<String>("Failed to load texture: " + path.string() + " - " + stbi_failure_reason());
    }
    ...
    if (result) {
        spdlog::debug("Texture loaded: {} ({}x{}, {} channels)", path.string(), width, height, channels);
    }
...
Result<void, String> Texture2D::create(const TextureSpec& spec, const void* data) {
    ...
    glTextureParameteri(m_textureId, GL_TEXTURE_MIN_FILTER, static_cast<GLint>(toGLFilter(spec.minFilter)));
    glTextureParameteri(m_textureId, GL_TEXTURE_MAG_FILTER, static_cast<GLint>(toGLFilter(spec.magFilter)));
    ...
    u32 mipLevels = spec.generateMipmaps ? 
        static_cast<u32>(std::floor(std::log2(std::max(m_width, m_height)))) + 1 : 1;
