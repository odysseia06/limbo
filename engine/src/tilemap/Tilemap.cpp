...
u32 Tilemap::addLayer(const String& name, i32 zOrder) {
    ...
    std::stable_sort(
        ...
        [](const TilemapLayer& a, const TilemapLayer& b) { return a.zOrder < b.zOrder; });
...
glm::ivec2 Tilemap::worldToTile(const glm::vec2& worldPos) const {
    return glm::ivec2(static_cast<i32>(std::floor(worldPos.x / m_tileWidth)),
                      static_cast<i32>(std::floor(worldPos.y / m_tileHeight)));
}

glm::vec2 Tilemap::tileToWorld(i32 x, i32 y) const {
    return glm::vec2((static_cast<f32>(x) + 0.5f) * m_tileWidth,
...
bool Tilemap::isValidTile(i32 x, i32 y) const {
    return x >= 0 && y >= 0 && static_cast<u32>(x) < m_width && static_cast<u32>(y) < m_height;
}
...
std::vector<glm::ivec2> Tilemap::getSolidTilesInRect(const glm::vec2& min,
