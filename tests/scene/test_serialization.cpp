...
TEST_CASE("SceneSerializer roundtrip with entities", "[scene][serialization]") {
    ...
    SECTION("Roundtrip preserves NameComponent") {
        ...
        world2.each<limbo::NameComponent>([&](auto, auto& name) {
            if (name.name == "TestEntity1") foundEntity1 = true;
            if (name.name == "TestEntity2") foundEntity2 = true;
        });
    ...
    SECTION("Roundtrip preserves SpriteRendererComponent") {
        ...
        world2.each<limbo::NameComponent, limbo::SpriteRendererComponent>(
            [&](auto, auto& name, auto& sprite) {
...
TEST_CASE("SceneSerializer preserves entity with multiple components", "[scene][serialization]") {
    ...
    world.addComponent<limbo::TransformComponent>(entity.id(),
        glm::vec3(5.0f, 10.0f, 15.0f),
        glm::vec3(0.0f, 0.0f, glm::radians(45.0f)),
        glm::vec3(2.0f, 2.0f, 2.0f));
    world.addComponent<limbo::SpriteRendererComponent>(entity.id(), glm::vec4(0.8f, 0.2f, 0.5f, 1.0f));
